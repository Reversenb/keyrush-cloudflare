...
3. **Rate Limiter** — จำกัด 30 request/นาที/IP (อ่าน IP จาก `cf-connecting-ip`)
...
|---|---|---|
| `User` | ผู้เล่น + ความคืบหน้าแบบเลเวล | `linuxLevel/Exp`, `windowsLevel/Exp`, `role`, `tokenVersion` |
...
| `PlayHistory` | บันทึกการเล่นแต่ละรอบ | `wpm`, `accuracy`, `earnedExp` (nullable) |
...
| `PUT` | `/api/admin/missions/:id` | แก้ไขด่าน |
...
│   └── rateLimit.ts      # rate limiter 30/นาที/IP
└── routes/
    ├── auth.ts           # Google login, logout
    ├── user.ts           # progress, profile, stats
    ├── mission.ts        # get mission, verify, reveal
    ├── leaderboard.ts    # จัดอันดับ
    ├── admin.ts          # จัดการด่าน (admin)
...
- [ ] Rate limiter แบบ persistent ด้วย Durable Objects (ปัจจุบันเก็บใน memory ต่อ isolate)
